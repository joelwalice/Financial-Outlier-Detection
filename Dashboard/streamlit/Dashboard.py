...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_file:
            df = pd.read_csv(uploaded_file)
            features = st.multiselect("Select Features for Outlier Detection", df.columns)
            methods = st.multiselect("Select Outlier Detection Methods", ["IQR Method", "Standard Deviation", "Z-Score", "LOF", "DBSCAN", "INFLO", "ABOD", "LDOF"])
    ...
    if uploaded_file:
        ...
        tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["IQR Method", "Standard Deviation","Z-Score", "LOF", "DBSCAN", "INFLO", "ABOD", "LDOF"])
